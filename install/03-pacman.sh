...
echo -e "\n[${BLUE}NOTE${RC}] - Adding extra spice to ${PACMAN_PATH}" 2>&1 | tee -a "${INSTALL_LOG}"
...
if grep -q '^#\[multilib\]' "${PACMAN_PATH}"; then
    ...
    if grep -q '^\[multilib\]' "${PACMAN_PATH}" && grep -A 1 '^\[multilib\]' "${PACMAN_PATH}" | grep -q '^Include'; then
        echo -e "${CL}[${GREEN}OK${RC}] - multilib repository enabled" 2>&1 | tee -a "${INSTALL_LOG}"
    else
        echo -e "${CL}[${RED}ERROR${RC}] - multilib repository was not enabled" 2>&1 | tee -a "${INSTALL_LOG}"
    fi
else
...
if sudo pacman -Syu --noconfirm --needed &>>"${INSTALL_LOG}"; then
    echo -e "${CL}[${GREEN}OK${RC}] - System updated" 2>&1 | tee -a "${INSTALL_LOG}"
else
    echo -e "${CL}[${RED}ERROR${RC}] - System update failed." 2>&1 | tee -a "${INSTALL_LOG}"
    exit 1
fi
