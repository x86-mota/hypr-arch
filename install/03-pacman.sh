...
echo -e "\n[${BLUE}NOTE${RC}] - Adding extra spice to ${PACMAN_PATH}" 2>&1 | tee -a "${InstallationLog}"
for MISC in "${MISC_OPTIONS[@]}"; do
    if grep -wq "^#${MISC}" "${PACMAN_PATH}"; then
        sudo sed -i "s/^#${MISC}/${MISC}/" "${PACMAN_PATH}"
        _IsAdded "^${MISC}" "${PACMAN_PATH}"
    else
        echo -e "[${BLUE}NOTE${RC}] - ${MISC} is already added to ${PACMAN_PATH}." 2>&1 | tee -a "${InstallationLog}"
    fi
done
...
if ! grep -wq '^ILoveCandy' "${PACMAN_PATH}"; then
    ...
    echo -e "[${BLUE}NOTE${RC}] - ILoveCandy is already added to ${PACMAN_PATH}." 2>&1 | tee -a "${InstallationLog}"
fi
...
if grep -q '^#\[multilib\]' "${PACMAN_PATH}"; then
    echo -e "[${BLUE}NOTE${RC}] - Enabling multilib repository..." 2>&1 | tee -a "${InstallationLog}"

    sudo sed -i "/^#\[multilib\]/,+1 s/^#//" ${PACMAN_PATH}

    if grep -q '^\[multilib\]' "${PACMAN_PATH}" && grep -A 1 '^\[multilib\]' "${PACMAN_PATH}" | grep -q '^Include'; then
        echo -e "${CL}[${GREEN}OK${RC}] - multilib repository enabled" 2>&1 | tee -a "${InstallationLog}"
    else
        echo -e "${CL}[${RED}ERROR${RC}] - multilib repository was not enabled" 2>&1 | tee -a "${InstallationLog}"
    fi
else
    echo -e "[${BLUE}NOTE${RC}] - multilib repository is already enabled." 2>&1 | tee -a "${InstallationLog}"
fi
...
if sudo pacman -Syu --noconfirm --needed &>>"${InstallationLog}"; then
    echo -e "${CL}[${GREEN}OK${RC}] - System updated" 2>&1 | tee -a "${InstallationLog}"
else
    echo -e "${CL}[${RED}ERROR${RC}] - System update failed." 2>&1 | tee -a "${InstallationLog}"
    exit 1
fi
